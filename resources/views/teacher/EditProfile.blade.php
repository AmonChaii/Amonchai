...
<head>
    ...
    <title>แก้ไขข้อมูลส่วนตัว (อาจารย์)</title>
    ...
    <main class="p-4 space-y-6">
        ...
        <form action="{{ route('teacher.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="flex justify-center">
                <div class="relative">
                    <div class="w-32 h-32 bg-gray-200 rounded-full border-4 border-blue-500 flex items-center justify-center overflow-hidden shadow-lg">
                        <img id="preview-image" src="{{ Auth::user()->image ? asset('storage/' . Auth::user()->image) : 'https://placehold.co/200x200/EBF5FF/3B82F6?text='.substr(Auth::user()->name, 0, 1) }}" class="w-full h-full object-cover">
                    </div>
                    <label for="image" class="absolute bottom-1 right-1 bg-white p-2 rounded-full shadow-md cursor-pointer hover:bg-gray-100">
            ...
            <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ Auth::user()->name }}" class="mt-1 w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                </div>

                <div>
                    <label class="text-sm font-medium text-gray-600">เบอร์โทรศัพท์</label>
                    <input type="tel" name="tel" value="{{ Auth::user()->tel }}" class="mt-1 w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>

                <div>
                    <label class="text-sm font-medium text-gray-600">ตำแหน่งทางวิชาการ</label>
                    <input type="text" name="position" value="{{ Auth::user()->position ?? '' }}" placeholder="เช่น ผศ.ดร." class="mt-1 w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>

                <div>
                    <label class="text-sm font-medium text-gray-600">อีเมล (แก้ไขไม่ได้)</label>
                    <input type="email" value="{{ Auth::user()->email }}" disabled class="mt-1 w-full p-3 border border-gray-300 rounded-lg bg-gray-50 text-gray-500">
                </div>
            </div>

            <button type="submit" class="w-full bg-blue-600 text-white font-semibold py-3 rounded-lg hover:bg-blue-700 shadow-md transition">บันทึกการเปลี่ยนแปลง</button>
        </form>
    </main>
